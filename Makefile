...
check-bundle-size: ui-build ## Check frontend bundle sizes against budgets
	...
	echo "  CSS:   $$CSS_SIZE bytes (budget: 35840 / 35KB)"; \
	echo "  JS:    $$JS_SIZE bytes (budget: 67584 / 66KB)"; \
	echo "  Icons: $$ICON_SIZE bytes (budget: 12288 / 12KB)"; \
	if [ $$CSS_SIZE -gt 35840 ]; then echo "FAIL: CSS exceeds 35KB budget" && exit 1; fi; \
	if [ $$JS_SIZE -gt 67584 ]; then echo "FAIL: JS exceeds 66KB budget" && exit 1; fi; \
